...
- **BigQuery sync**: `/api/cron/sync-bigquery` (scheduled every minute via [vercel.json](vercel.json)) pushes tickets to the `helpdesk_data.tickets_analytics` table; the analytics dashboard reads from BigQuery.
