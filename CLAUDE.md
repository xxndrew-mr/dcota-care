...
- **File uploads**: [api/go-upload.go](api/go-upload.go) (Go, Vercel function) issues presigned PUT URLs to Cloudflare R2; tickets store only attachment metadata in `TicketDetail.attachments_json`. Auth is a short-lived HMAC token: the client fetches one from `/api/upload-token` (session-checked), then sends it as the `X-Upload-Token` header; the Go function verifies it locally with `UPLOAD_SIGNING_SECRET` (falls back to `NEXTAUTH_SECRET`) — no cross-function HTTP callback.
